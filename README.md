...
| 项目 | 路径 | 说明 |
|------|------|------|
| Hot-pluggable Agent Harness | [`projects/hotplug-harness/`](projects/hotplug-harness/) | 工具/模型/策略插件热加载；Harness 拥有 Loop（Budget / Allowlist / Circuit）；FastAPI + Vite **Q&A 聊天控制台**（消息驱动 Run，旁侧看控制面）；`python demo.py` → `ALL_OK` |
