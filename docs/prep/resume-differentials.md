...
> 配套模拟面：`docs/prep/mock-interviews/round-0{1,2,3}-*.md` · 课纲：`03-modules/**`

**一句话定位（可放简历摘要）**  
四年 Java 背景转向 Agent 平台：能把 **网关抬权 → 编排续跑 → 领域事务** 拆清，并用故障注入 + Eval 门禁证明「不双写、不串租、不靠 Prompt 当安全边界」。


> 扩展投递条目（+12）：见 [/docs/prep/resume-bullets-extended.md](/docs/prep/resume-bullets-extended.md)。
...
| 谁开事务？ | 只在 Java 领域服务 | `docs/durable-runtime-research.md` §2 |
...
| 项目名建议 | 一句话 | 路径 |
|------------|--------|------|
| 多租户企业 Agent 控制面（Capstone） | 五段演示：四键 / 幂等 Outbox / RAG ACL / 熔断 / Trace→Eval | `03-modules/A9-capstone/` |
| Durable Runtime 对照（Temporal） | Signal→Activity 幂等；超管 DB bypass 挂科 | `04-frontier/B1-durable-runtime/` |
| 最小 A2A | 身份·幂等·超时取消 | `04-frontier/B2-a2a/` |

模拟面记录（可放「面试准备」或内推附录，一般不放对外简历正文）：  
`docs/prep/mock-interviews/`（R1–R3 已通过）。
...
- [ ] O10 七段脱稿计时 ≤ 2min（R3）
- [x] @Java高级架构师 审过①归属链表述；[x] @Agent工程师 审过②③
...
| Agent工程师 | **通过** | ②③表述到位：硬门禁/Trace→fixture/incomplete 禁 holdout；MCP 非安全边界 + A2A 身份·幂等·取消负例。非阻塞：②可半句点名「版本三元组 eval_set×agent_config×model」（R2 Q12），字数紧可省略。 |
