...
> 用途：驱动 [能力矩阵](capability-matrix-3to5y.md)、[进阶课纲](curriculum-advanced-3to5y.md)、[深挖题 v2](interview-deep-dive-v2.md)。  
> 性质：**公开岗位描述的共性归纳**（猎聘等可复核链接），**不是** Boss 直聘私有抓取，也不代表已投递结果。  
> 禁忌：不编造薪资统计、编制、通过率；下文若出现岗位页上的薪资区间，仅作「该条 JD 原文可见信息」，不作市场均值。

入门岗画像仍见仓库旧文 `docs/boss-jd-research.md`。**3–5 年岗已上移到 Runtime / Harness / 长任务一致性 / 沙箱 / Eval 回归 / 私有化。**
...
1. **Runtime State / Checkpoint / Rollback（及 Commit）** — 长链任务可恢复；状态机不是「内存里的 dict」。
2. **长任务续跑 + HITL** — 断点、人工接管、失败恢复。
3. **Tool Sandbox + Guardrail + 审计** — 工具封装内部系统时默认危险。
4. **Eval / Regression / Tracing** — 质量可量化；回归能拦住；链路可追。瓴知明确到「四层」。
5. **Java + MQ + 向量库 + 私有化** — 企业交付不是纯 Python notebook。
6. **MCP / Skill 注册与版本** — 工具/技能平台化。
7. **上下文工程与所有权** — 能独立扛方向，而不是只会接框架 API。
...
完整口试见 [过不了就别去面](interview-deep-dive-v2.md#gate)。
...
差距清单固化为课纲 [10 个死亡坑](curriculum-advanced-3to5y.md#holes)。
